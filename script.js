class EduLearnAI {
    constructor() {
        this.apiKey = localStorage.getItem(config.storage.apiKey) || '';
    ...
    bindEvents() {
        ...
        if (this.exportPdfBtn) {
            this.exportPdfBtn.addEventListener('click', () => this.exportContentAsPDF());
        }
        ...
        this.apiKeyInput.addEventListener('input', () => {
            ...
            localStorage.setItem(config.storage.apiKey, this.apiKey);
        });
    ...
    async exportContentAsPDF() {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF();

        const courseTitle = this.courseTitleInput.value.trim() || 'Course Content';
        const margin = 10;
        let y = margin;

        doc.setFontSize(18);
        doc.text(`Educational Content Package: "${courseTitle}"`, margin, y);
        y += 10;

        doc.setFontSize(12);
        doc.text('Generated by Edu-LearnAI', margin, y);
        y += 10;
        ...
        addSection('COURSE OBJECTIVE:', this.courseObjective.textContent || 'Content not available');
        addSection('SYLLABUS:', this.courseSyllabus.textContent || 'Content not available');
        addSection("LEARNING OUTCOMES (Bloom's Taxonomy):", '');
        addSection('Knowledge:', this.knowledgeOutcome.textContent || 'Content not available');
        addSection('Comprehension:', this.comprehensionOutcome.textContent || 'Content not available');
        addSection('Application:', this.applicationOutcome.textContent || 'Content not available');
        addSection('SUGGESTED ASSESSMENTS:', this.suggestedAssessments.textContent || 'Content not available');
        addSection('RECOMMENDED READINGS:', this.recommendedReadings.textContent || 'Content not available');

        doc.save(`${courseTitle.replace(/\s+/g, '_')}_EduLearnAI.pdf`);
    }
